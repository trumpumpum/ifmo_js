...
console.log(`Введите пароль с учётом следующих требований:
- длина от 9 символов;
- содержит обязательно английские буквы верхнего и нижнего регистра;
- содержит более двух цифр;
- содержит обязательно один из неалфавитных символов (например, !, $, #, %).`);
...
if (/.{9,25}/.test(pswd)) {
  if (/[A-Z]/.test(pswd)) { //Поправить
    if (/[a-z]/.test(pswd)) {
        if (pswd.match(/\d/g).length > 2) {
          if (/\W/.test(pswd)) {
          	console.log("Пароль удовлетворяет всем требованиям");
          } else {
          	console.log("Пароль должен содержать минимум 1 неалфавитный символ");
          }
        } else {
          console.log("Пароль должен содержать более 2 цифр");
        }   
      } else {
        console.log("Пароль должен содержать английские буквы также нижнего регистра");
      }
  } else {
    console.log("Пароль должен содержать английские буквы также ВЕРХНЕГО регистра");
  }
} else {
  console.log("Длина указанного пароля меньше 9 символов");
}
// 4** Нечёткий поиск:
